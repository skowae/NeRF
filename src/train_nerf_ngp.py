...
def main():
   ...
   train(model, device, optimizer, scheduler, dataset, batch_size, epochs, N_samples, near, far, out_dir, perturb)
...
def render_image(model, rays_o, rays_d, device, perturb=True, near=None, far=None, N_samples=64, chunk_size=2048, scene_center=None, scene_scale=None):
...
def train(model, device, optimizer, scheduler, dataset, batch_size, epochs, N_samples, near, far, results_dir, perturb):
   """Training loop
   ...
   try: 
      ...
      for epoch in range(1, epochs):
         ...
         for img, pose, focal, pp, img_wh in tqdm(dataloader, desc=f"Epoch {epoch} [Train]"):
            ...
            # Render image
            rgb_map = render_image(model, rays_o, rays_d, device, perturb=perturb, near=near, far=far, scene_center=scene_center, scene_scale=scene_scale)
         ...
         #    # Render image
         #    rgb_map = render_image(model, rays_o, rays_d, device, perturb=False, near=near, far=far, scene_center=scene_center, scene_scale=scene_scale)
         ...
         if train_ssim > best_val_ssim:
            print(f"New best SSIM = {train_ssim}")
            best_val_ssim = train_ssim
            
         if train_psnr > best_val_psnr:
            print(f"New best PSNR {train_psnr}")
         ...
         if epoch % save_period == 0 and first_val_batch is not None:
            ...
            rgb_map = render_image(model, rays_o, rays_d, device, perturb=False, 
                                   near=near, far=far, scene_center=scene_center, 
